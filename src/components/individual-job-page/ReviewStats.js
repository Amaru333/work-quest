  ...
  return (
    <div>
      <h2 className="text-3xl font-medium mb-4">
        {JOB_DESCRIPTION_PAGE_STRINGS.companyReviews[lang]}
      </h2>
      <div className="grid grid-cols-6 gap-x-4">
            ...
            <div className="flex">
              {[1, 2, 3, 4, 5].map((star) => (
                <Image
                  ...
                  width={20}
                  height={20}
                  src={`/icons/star-${star <= reviewData?.average ? "fill" : "mute"}.svg`}
                  alt="star"
                />
              ))}
            </div>
        ...
        <div className="col-span-2">
          {[5, 4, 3, 2, 1].map((star) => {
            const highestStarDistribution = Math.max(
              ...Object.values(reviewData?.distribution || { a: 10 })
            );
            return (
              <div key={star} className="flex items-center gap-x-1 text-[10px] mb-1">
                ...
                <div
                  className="bg-primary-100 h-1 rounded-full mx-1"
                  style={{
                    width: `${
                      (reviewData?.distribution?.[star] * 100) / highestStarDistribution || 0.1
                    }%`,
                  }}
                ></div>
                <p>{currencyShort(reviewData?.distribution?.[star] || 0)}</p>
              </div>
            );
          })}
        </div>
        <div className="col-span-2">
          <div className="flex text-[10px] mb-1 items-center gap-x-2">
            ...
            <div className="w-full h-1 bg-gray-300 rounded-full">
              <div
                className="h-1 bg-primary-100 rounded-full"
                style={{ width: `${(reviewData?.feedbacks?.time * 100) / reviewData?.count}%` }}
              ></div>
            </div>
            <p>{((reviewData?.feedbacks?.time * 100) / reviewData?.count || 0).toFixed(0)}%</p>
          </div>
          <div className="flex text-[10px] mb-1 items-center gap-x-2">
            ...
            <div className="w-full h-1 bg-gray-300 rounded-full">
              <div
                className="h-1 bg-primary-100 rounded-full"
                style={{ width: `${(reviewData?.feedbacks?.benefit * 100) / reviewData?.count}%` }}
              ></div>
            </div>
            <p>{((reviewData?.feedbacks?.benefit * 100) / reviewData?.count || 0).toFixed(0)}%</p>
          </div>
          <div className="flex text-[10px] mb-1 items-center gap-x-2">
            ...
            <div className="w-full h-1 bg-gray-300 rounded-full">
              <div
                className="h-1 bg-primary-100 rounded-full"
                style={{ width: `${(reviewData?.feedbacks?.pay * 100) / reviewData?.count}%` }}
              ></div>
            </div>
            <p>{((reviewData?.feedbacks?.pay * 100) / reviewData?.count || 0).toFixed(0)}%</p>
          </div>
          <div className="flex text-[10px] mb-1 items-center gap-x-2">
            ...
            <div className="w-full h-1 bg-gray-300 rounded-full">
              <div
                className="h-1 bg-primary-100 rounded-full"
                style={{
                  width: `${(reviewData?.feedbacks?.environment * 100) / reviewData?.count}%`,
                }}
              ></div>
            </div>
            <p>
              {((reviewData?.feedbacks?.environment * 100) / reviewData?.count || 0).toFixed(0)}%
            </p>
          </div>
        </div>
      </div>
