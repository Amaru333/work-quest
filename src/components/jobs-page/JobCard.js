...
function JobCard({ job, lang }) {
  ...
  return (
    <div className="rounded-2xl p-1 bg-white border border-gray-300 flex flex-col">
      <div className="rounded-xl p-3 flex-1 flex flex-col" style={{ backgroundColor: job?.backgroundColor }}>
        ...
        <div className="flex items-center justify-between mt-2 gap-x-2 flex-1">
          ...
          <Image src={job?.company?.logo} width={72} height={72} alt={job?.company?.name} className="rounded-full bg-white" />
        </div>
        <div className="flex items-center flex-wrap gap-2 mt-6 mb-4">
          {job?.tags?.map((tag) => (
            <Badge key={tag[lang]} label={tag[lang]} />
          ))}
        </div>
      </div>
