        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="bg-primary-300 text-white max-w-40 px-0 py-0">
            {LANGUAGE_LIST.map((language) => (
              <DropdownMenuItem
                key={language.code}
                className={`w-full text-left py-3 px-6 hover:bg-primary-200 transition-all duration-200 cursor-pointer ${
                  language.code === lang ? "bg-primary-200" : "bg-primary-300"
                }`}
                onClick={() => dispatchLanguage(language)}
              >
                {language.name}
              </DropdownMenuItem>
            ))}
