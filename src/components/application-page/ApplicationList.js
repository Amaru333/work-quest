...
function ApplicationList({ list = [] }) {
  ...
  const statuses = [
    ...
    },
    {
      ...
      applications: list?.filter((app) => app.status === "processing"),
  ...
  return (
    <div className="col-span-3 border-r border-gray-400 pt-6">
      <p className="text-3xl font-medium mb-2">{MY_APPLICATIONS_PAGE_STRINGS.myApplications[lang]}</p>
      <div className="pr-4">
        <Accordion type="multiple" collapsible defaultValue={MY_APPLICATIONS_PAGE_STRINGS.approved[lang]}>
                  ...
                  {status?.applications?.map((app, index) => (
                    <div key={index} className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <Image src={app?.company?.logo} width={50} height={50} className="rounded-full" alt={app?.company?.name} />
                        <div>
                          <p className="text-base font-semibold leading-5">
                            {app?.position?.[lang]} - {app?.company?.name}
                          </p>
                          <p className="text-xs font-semibold text-gray-500">
                            {COMMON_STRINGS.applied[lang]} {getRelativeTime(app?.applied_on, lang)}
                          </p>
                        </div>
                      </div>
